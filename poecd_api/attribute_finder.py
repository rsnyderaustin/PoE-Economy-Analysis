...
class PoecdAttributeFinder:
    ...
    def _throw_unavailable_mod_error(item_mod: ItemMod, poecd_mod: PoecdMod):
        logging.info("\n\n--------------- Item Mod ----------------")
        api_trade_skills = [sub_mod.sanitized_mod_text for sub_mod in item_mod._sub_mods]
    ...
    def get_poecd_mod_attributes(self, item_mod: ItemMod) -> PoecdModAttributes | None:
        ...
        if not poecd_mod_id:
            self._throw_no_match_error(item_mod)
        ...
        try:
            ...
            self._throw_unavailable_mod_error(item_mod=item_mod,
                                              poecd_mod=poecd_mod)
