...
from poecd_api.manager import PoecdEndpoint, PoecdApiManager
...
class PoecdDataManager:
    ...
    def __init__(self, repull_poecd_source: bool = False):
        files_manager = file_management.FilesManager()

        if repull_poecd_source:
            api_manager = PoecdApiManager()
            files_manager.file_data[FileKey.POECD_BASES] = api_manager.pull_data(PoecdEndpoint.BASES)
            files_manager.file_data[FileKey.POECD_MODS] = api_manager.pull_data(PoecdEndpoint.MODS)

        self.bases_data = files_manager.file_data[FileKey.POECD_BASES]
        self.stats_data = files_manager.file_data[FileKey.POECD_STATS]
        ...
        if files_manager.has_data(key=FileKey.POECD_MODS):
            mods = files_manager.file_data[FileKey.POECD_MODS]
        else:
            mods = self._build_mods()
            files_manager.file_data[FileKey.POECD_MODS] = mods
            files_manager.save_data(keys=[FileKey.POECD_MODS])

        self.atype_data_managers = self._fill_atype_managers(mods=mods)

    def fetch_atype_manager(self, atype: str):
        return self.atype_data_managers[atype]

    def _fill_atype_managers(self, mods):
        atype_managers = dict()
        for atype_id in self.valid_atype_ids:
            atype_name = self.atype_id_to_atype_name[atype_id]
            atype_managers[atype_name] = ATypeDataManager(
                atype_id,
                atype_name,
